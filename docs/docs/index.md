...
There are two ways to configure your API keys in bolt:
...
- On **Mac**, you can find the file at `[your name]/bolt/.env.example`.
...
To keep your local version of bolt up to date with the latest changes, follow these steps for your operating system:
...
This ensures that you're running the latest version of bolt and can take advantage of all the newest features and bug fixes.

---

## Adding New LLMs:

To make new LLMs available to use in this version of bolt, head on over to `app/utils/constants.ts` and find the constant MODEL_LIST. Each element in this array is an object that has the model ID for the name (get this from the provider's API documentation), a label for the frontend model dropdown, and the provider.
